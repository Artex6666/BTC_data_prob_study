"""
sol_curve_optimizer_v1.py — Optimizer M5 SOL

Dérivé de eth_curve_optimizer_v1.py.
Slopes/intercepts/vol seuils calibrés pour SOL (~$83, vol1h médiane ~$0.97).
  thresh = slope * remain_s + intercept  (en $)
  slope=0.005, remain=60s → thresh=$0.30 (0.36% du spot)
"""
...
OUT_ROOT = ROOT / "god_curve" / "sol_curve_results_v1"

DEFAULT_CSV_PATHS = [
    str(ROOT / "Datas" / "csv" / "SOL.csv"),
    str(ROOT / "reportLive" / "safeChase" / "SOL.csv"),
]

# Grilles calibrées SOL (~$83)
# thresh à remain=60s : slope*60. Ex: 0.005*60=$0.30 (~0.36%)
SLOPE_GRID     = [0.001, 0.002, 0.005, 0.010, 0.020, 0.050]
INTERCEPT_GRID = [0.0, 0.05, 0.10, 0.20]
CAP_GRID       = [0.55, 0.65, 0.75, 0.85, 0.90]

# Vol range/net : médiane vol1h=$0.97, p90=$1.66
VOL_RANGE_GRID = [0.30, 0.60, 1.00, 1.80, 3.00]
VOL_NET_GRID   = [0.20, 0.40, 0.80, 1.50]
...
# VRS base calibrée SOL
VRS_BASE_SOL   = {1.0: 0.5, 2.0: 1.0, 4.0: 2.0}
CNET_THRESH    = [0.05, 0.10, 0.20, 0.50]
...
def build_configs():
    ...
    # G : VRS
    for lb, base in VRS_BASE_SOL.items():
        for slope in [0.002, 0.005, 0.010]:
            for cap in CAP_GRID:
...
def fmt_row(i, r, days):
    curve_s = f"sl={r['slope']:.5f} int={r['intercept']:.3f}"
    ...
    if r.get('vol_thresh') is not None:
        ...
        if vt in ('pct_range', 'pct_net'):
            ...
            thr_s = f"${r['vol_thresh']:.3f}"
        vol_s = f"{vt[:4]}{r['vol_lb_h']:g}h>{thr_s}"
    elif r.get('vrs_enabled'):
        vol_s = f"vrs{r['vrs_lb']:g}h b={r['vrs_base']:.2f} g={r['vrs_g_min']:.2f}-{r['vrs_g_max']:.2f}"
    elif r.get('cnet_n'):
        vol_s = f"cnet{r['cnet_n']}c>${r['cnet_thresh']:.3f}"
    else:
        vol_s = "              -"
...
def main():
    parser = argparse.ArgumentParser(description="SOL Curve Optimizer v1")
    ...
    out_dir = OUT_ROOT / f"sol_v1_{ts_str}"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"=== SOL Curve Optimizer v1 ===\n")
    ...
    txt = out_dir / f"sol_curve_results_v1_{ts_str}.txt"
    ...
    if not args.no_charts and all_results:
        ...
        generate_charts(
            ...
            n_top=12, title_prefix="SOL Curve v1",
