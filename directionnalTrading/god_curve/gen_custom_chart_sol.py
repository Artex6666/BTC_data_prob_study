"""
Génère deux overlay charts SOL avec configs manuelles :
  - overlay_all.png    : toute la période (Datas/csv/SOL.csv + safeChase)
  - overlay_last4d.png : période récente (reportLive SOL.csv)
...
OUT_DIR = Path(__file__).resolve().parent / "custom_chart_sol"
...
# Calibration SOL (~$83) : slope*60 = thresh à 60s
# Ex: slope=0.005 → thresh=$0.30 à remain=60s (~0.36% du spot)
BASE_CONFIGS = [
    # rank 1 RF — vol_net_lin sl=0.005 cap=0.75 net1h>$0.20
    dict(label='vol_net_lin', curve='linear',
         slope=0.005, intercept=0.0,
         A_exp=None, tau=None,
         eq_cap=0.75, max_losses_cb=None,
         vol_lb_h=1.0, vol_thresh=0.20, vol_type='net'),

    # rank 2 RF — vrs_lin sl=0.010 cap=0.75 vrs1h b=0.50 g=0.50-1.50
    dict(label='vrs_lin', curve='linear',
         slope=0.010, intercept=0.0,
         A_exp=None, tau=None,
         eq_cap=0.75, max_losses_cb=None,
         vol_lb_h=None, vol_thresh=None,
         vrs_enabled=True, vrs_lb=1.0, vrs_base=0.50,
         vrs_g_min=0.50, vrs_g_max=1.50),

    # rank 11 RF — cnet_lin sl=0.005 cap=0.85 cnet2c>$0.10
    dict(label='cnet_lin', curve='linear',
         slope=0.005, intercept=0.0,
         A_exp=None, tau=None,
         eq_cap=0.85, max_losses_cb=None,
         vol_lb_h=None, vol_thresh=None,
         cnet_n=2, cnet_thresh=0.10),

    # rank 1 PnL — vol_rng_lin sl=0.001 cap=0.55 range4h>$1.00
    dict(label='vol_rng_lin', curve='linear',
         slope=0.001, intercept=0.0,
         A_exp=None, tau=None,
         eq_cap=0.55, max_losses_cb=None,
         vol_lb_h=4.0, vol_thresh=1.00, vol_type='range'),

    # rank 1 consistance — vol_pctr_lin sl=0.005 cap=0.75 pct_range1h>0.50%
...
# ── Moteur principal ──────────────────────────────────────────────────────────
def run_chart(csv_paths, out_dir, title_prefix, last_days=None, day=None):
    ...
    for tf_floor, bid_up, bid_down, ask_up, ask_down in TIMEFRAMES:
        try:
            ...
            elif last_days is not None and cts:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Custom god_curve charts (SOL)")
    parser.add_argument(
        "-4d",
        "--last-4d",
        action="store_true",
        dest="last_4d_only",
        help="Générer uniquement la chart last4d (filtre 4 jours), pas la période full.",
    )
    parser.add_argument(
        "--day",
        type=str,
        default=None,
        metavar="YYYY-MM-DD",
        help="Un jour UTC (ce). Sortie custom_chart_sol/day_*/.",
    )
    ...
    if args.day:
        ...
        print(f"\n=== SOL jour UTC {d} ===", flush=True)
        run_chart(CSV_FULL, sub, f"Custom SOL - {d}", day=d)
        print(f"\nOverlay -> {sub / 'overlay_all.png'}")
    else:
        if not args.last_4d_only:
            print("\n=== SOL Chart 1 : toute la période ===")
            run_chart(CSV_FULL, OUT_DIR / "all", "Custom SOL - Full")

        print("\n=== SOL Chart 2 : 4 derniers jours ===")
        run_chart(CSV_FULL, OUT_DIR / "last4d", "Custom SOL - Last 4d", last_days=4)
